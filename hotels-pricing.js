(() => {
  ...
  function render(k){
    ...
    body.innerHTML=`<tr><td><b>MANAGED · Monthly</b><br><span class="fine">Lower commitment</span></td>${p.setup.map((x,i)=>`<td><b>${i===2?'From ':''}${money(p,x)} today</b><br>then ${money(p,p.mo[i])}/mo<br><span class="fine">Year 1: ${i===2?'from ':''}${money(p,x+p.mo[i]*12)}</span></td>`).join('')}</tr><tr><td><b>MANAGED · Annual ⭐</b><br><span class="fine">Best value</span></td>${annual.map((x,i)=>`<td><b>${i===2?'From ':''}${money(p,x)} today</b><br><span class="fine">${money(p,p.setup[i])} setup + first year ${money(p,p.yr[i])}</span><br>then ${i===2?'from ':''}${money(p,p.yr[i])}/yr</td>`).join('')}</tr><tr><td><b>Annual saving</b><br><span class="fine">vs paying monthly</span></td>${save.map((x,i)=>`<td>${i===1?'<b>':''}${money(p,x)}${i===2?'+':''}/year${i===1?'</b>':''}</td>`).join('')}</tr><tr><td><b>ONE-TIME · Pay once</b><br><span class="fine">No subscription</span></td>${p.once.map((x,i)=>`<td><b>${i===2?'From ':''}${money(p,x)} once</b></td>`).join('')}</tr>`;
